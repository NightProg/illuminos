...
impl Ext2BlockGroupDescriptor {
    ...
    pub fn from_disk(superblock: Ext2SuperBlock, disk: &mut impl Disk) -> Option<Vec<Self>> {
        let group_count = superblock.get_block_group_count() as usize;
        let block_size = superblock.block_size() as usize;

        let start_block = if block_size == 1024 { 2 } else { 1 };
        let mut raw = Vec::new();

        let bgd_per_block = block_size / size_of::<Ext2BlockGroupDescriptor>();
        let blocks_needed = (group_count + bgd_per_block - 1) / bgd_per_block;

        for i in 0..blocks_needed {
            raw.extend(superblock.read_block(start_block + i as u32, disk));
        }

        let mut v = Vec::with_capacity(group_count);
        for i in 0..group_count {
            let off = i * size_of::<Ext2BlockGroupDescriptor>();
            let bgd = unsafe {
                (raw[off..].as_ptr() as *const Ext2BlockGroupDescriptor)
                    .as_ref()
                    .cloned()?
            };
            v.push(bgd);
...
impl DirectoryEntry {
    pub fn from_slice(buf: &[u8]) -> Option<(Self, usize)> {
        ...
        let name = &buf[8..8 + name_len as usize];
        let mut name_arr = [0u8; 255];
        name_arr[..name_len as usize].copy_from_slice(name);
    ...
    pub fn flush(&self) -> Vec<u8> {
        ...
        b.push(self.name_length);
        b.push(self.file_type);
        b.extend_from_slice(&self.name[..self.name_length as usize]);
        b
    }
}
    ...
    T: Disk + Clone,
{
    ...
    pub fn read_inode(&mut self, inode_number: u32) -> Option<Inode> {
        if let Some(cached) = self.inode_cache.get(&inode_number) {
            return Some(*cached);
        }

        let inode = self.read_inode_no_cached(inode_number)?;

        self.inode_cache.insert(inode_number, inode);

        Some(inode)
    }

    pub fn read_inode_no_cached(&mut self, inode_number: u32) -> Option<Inode> {
        let inode_size = self
            .super_block_ext
    ...
    pub fn read_file(&mut self, inode: Inode) -> Result<Vec<u8>, Error> {
        let block_size = self.super_block.block_size();
    ...
    pub fn read_block(&mut self, block_id: u32) -> Vec<u8> {
        ...
        let data = self.super_block.read_block(block_id, &mut self.disk);
        self.block_cache.insert(block_id, data.clone());
        data
    }

    pub fn read_inode_by_name(&mut self, dir_inode: u32, name: &str) -> Result<u32, Error> {
        let dirs = self.read_directory_inode(dir_inode)?;
        for dir in dirs {
    ...
    pub fn read_path(&mut self, path: Path) -> Result<u32, Error> {
        if path.is_relative() {
            return Err(Error::CantUseRelPath);
        }

        let dirs = self.read_directory_inode(2)?;
        ...
        for byte_index in 0..bitmap.len() {
            ...
            for bit_index in 0..8 {
                if (byte & (1 << bit_index)) == 0 {
                    ...
                    let block_absolute =
                        self.super_block.block_group_count * block_grp + block_index_within_group;
    ...
    pub fn insert_inode(
        ...
        let mut parent_inode = self
            .read_inode(parent_inode_number)
            .ok_or(super::Error::FileNotFound)?;
        let mut dir = self.read_directory_inode(parent_inode_number)?;
        let inode_id = self.allocate_inode().ok_or(super::Error::NotEnoughSpace)?;
        let mut name_arr = [0; 255];
    ...
    pub fn insert_inode2(
        ...
        let inode_id = self.allocate_inode().ok_or(Error::NotEnoughSpace)?;
        let mut name_arr = [0; 255];
        name_arr[..name.len()].copy_from_slice(name.as_bytes());
        let mut new_entry = DirectoryEntry {
            inode: inode_id,
        ...
        while offset < file.len() {
            let mut entry = DirectoryEntry::from_slice(&file[offset..]).unwrap().0;
            if entry.inode == 0 {
...
impl<T: Disk + Clone + 'static> super::FileSystem for Ext2FS<T> {
    ...
    fn read_directory_lazy(&mut self, path: Path) -> Result<Directory, Error> {
        let inode_id = self.read_path(path.clone())?;
        let inode = self.read_inode(inode_id).ok_or(Error::FileNotFound)?;

        if !inode.is_dir() {
            return Err(Error::NotADirectory);
        }

        let raw_entries = self.read_directory_inode(inode_id)?;
        ...
        for entry in raw_entries {
            ...
            entries.push(self.read_depth_1_dir_entry(entry, path.clone())?);
        }

        Ok(Directory::new(
            entries,
            path.components().last().unwrap_or(&"/".to_string()).clone(),
            path,
        ))
    }
