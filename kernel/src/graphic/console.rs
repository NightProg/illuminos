...
// use crate::thread::process::{Process, ProcessArguments};
// use crate::thread::{SCHEDULER, yield_now};
// use crate::{dbg, println, thread};
// use alloc::boxed::Box;
...
// pub struct Console<'a, P: PsfFont> {
//     text_buffer: TextBuffer<'a, P>,
...
// impl<'a, P: PsfFont> Console<'a, P> {
//     pub fn new(framebuffer: &'a mut RawFrameBuffer, font: &'a P) -> Self {
...
//             "exec" => {
//                 if commands.len() < 2 {
//                     self.write_str("Usage: exec <executable> <args> \n");
//                     return;
//                 }
//                 let executable = commands[1];
//                 let args = commands[2..].iter().map(|s| s.to_string()).collect();
//                 let inode = GLOBAL_CONTEXT.fs.lock().lookup(executable);
//                 match inode {
//                     Ok(inode) => {
...
//                             let is_task_running_clone = self.is_task_running.clone();
//                             process.add_exit_callback(move || {
//                                 is_task_running_clone.store(false, Ordering::SeqCst);
//                             });

//                             process.add_to_processes();

//                             let task = thread::process::spawn(process.pid, rip).unwrap();
//                             self.is_task_running.store(true, Ordering::SeqCst);

//                             /*  let exit_code = thread::process::get_exit_code(process.pid).unwrap();

//                             if exit_code == 0 {
//                                 self.write_str("Process exited successfully.\n");
//                             } else {
...
//             }
//             "lscd" => {
//                 let entries = GLOBAL_CONTEXT.fs.lock().list_entries(&*self.current_dir);
//                 match entries {
//                     Ok(files) => {
...
//                             }
//                             let open_file = GLOBAL_CONTEXT.fs.lock().open_file(
//                                 device,
...
//                                     GLOBAL_CONTEXT.fs.lock().mounts.push(crate::fs::Mount {
...
//                         path = &current_path;
//                     }
//                     let read_inode = GLOBAL_CONTEXT.fs.lock().lookup(path);
...
//                         path = &current_path;
//                     }
//                     let write_inode = GLOBAL_CONTEXT.fs.lock().lookup(path);
...
//                     let result = GLOBAL_CONTEXT.fs.lock().mkdir(path);
//                     match result {
//                         Ok(_) => {
...
//                         path = &current_path;
//                     }
//                     let result = GLOBAL_CONTEXT.fs.lock().create_file(path);
//                     match result {
//                         Ok(_) => {
...
//                     let result = GLOBAL_CONTEXT.fs.lock().umount(mount_point);
//                     match result {
//                         Ok(_) => {
...
//             KEYBOARD.lock().process_keyevent(key)
//         });
...
// impl<P: PsfFont> core::fmt::Write for Console<'_, P> {
...
//     let framebuffer = &mut *GLOBAL_CONTEXT.framebuffer.lock();
//     let mut console = Console::new(framebuffer, &*FONT_DEFAULT);

//     loop {
//         while let Some(event) =
//             x86_64::instructions::interrupts::without_interrupts(|| GLOBAL_CONTEXT.pop_key())
//         {
